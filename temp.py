...
class DETR(nn.Module):
    """DETR model with ResNet-18 backbone"""
    def __init__(self, num_classes=3, num_queries=100, hidden_dim=256):
        super().__init__()
...
def load_detr_model(detr_path, backbone_path):
    """Load DETR model with ResNet-18 backbone and custom weights"""
    try:
        print(f"Loading DETR model from {detr_path} with backbone from {backbone_path}")
        model = DETR(num_classes=3)
        
        # Inspect checkpoints
        inspect_checkpoint(detr_path)
        if backbone_path:
            inspect_checkpoint(backbone_path)
        
        # Load backbone weights if provided
        if backbone_path:
            print(f"\nLoading backbone from {backbone_path}")
            bb_checkpoint = torch.load(backbone_path, map_location=device)
            if isinstance(bb_checkpoint, dict):
                bb_state_dict = bb_checkpoint.get('model_state_dict') or bb_checkpoint.get('state_dict') or bb_checkpoint
            else:
                bb_state_dict = bb_checkpoint
            bb_state_dict = {k.replace('module.', ''): v for k, v in bb_state_dict.items()}
            missing, unexpected = model.backbone.load_state_dict(bb_state_dict, strict=False)
            print(f"Backbone loaded. Missing keys: {len(missing)}, Unexpected keys: {len(unexpected)}")
            if missing:
        ...
        print(f"Successfully loaded {len(filtered_dict)} compatible DETR parameters")
...
def predict_with_detr(model, image_path, confidence_threshold=0.1):
    ...
    for i in range(len(pred_logits)):
        ...
        if max_prob > confidence_threshold and class_idx < 3:  # Ensure class index is within 0-2
...
if __name__ == "__main__":
    backbone_path = "pretrained_models/resnet18_simclr_kitti_blurred.pth"
    detr_path = "pretrained_models/detr_r50.pth"
    output_dir = "output_viz/clean"
    test_dir = r"D:\harsh\Saarland University\Sem 2\HLCV\hlcv_project\data\blurred"
    
    # Load model
    model = load_detr_model(detr_path, backbone_path)
    
    # Class names for your 3-class KITTI dataset
    ...
    for image_path in images:
        # Run inference
        predictions = predict_with_detr(model, image_path, confidence_threshold=0.1)
